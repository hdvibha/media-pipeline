...
exports.checkTampering = checkTampering;
 ...
 */
async function checkTampering(filePath) {
    ...
    try {
        ...
        const original = await (0, sharp_1.default)(filePath)
            .resize({ width: targetWidth, withoutEnlargement: true })
            .raw()
            .toBuffer({ resolveWithObject: true });
        const recompressedBuf = await (0, sharp_1.default)(filePath)
            .resize({ width: targetWidth, withoutEnlargement: true })
            .jpeg({ quality: 90 })
            .toBuffer();
        const recompressed = await (0, sharp_1.default)(recompressedBuf).raw().toBuffer({ resolveWithObject: true });
