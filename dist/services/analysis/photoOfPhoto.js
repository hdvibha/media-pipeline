...
exports.checkPhotoOfPhoto = checkPhotoOfPhoto;
 ...
 */
async function checkPhotoOfPhoto(filePath) {
    const start = Date.now();
    const img = (0, sharp_1.default)(filePath);
...
function stdev(buf) {
    ...
    let variance = 0;
    for (let i = 0; i < buf.length; i++)
        variance += (buf[i] - mean) ** 2;
    variance /= buf.length;
    return Math.sqrt(variance);
}
//# sourceMappingURL=photoOfPhoto.js.map